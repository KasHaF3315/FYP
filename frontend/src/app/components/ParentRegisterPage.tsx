import { User, Mail, Hash, AlertCircle, ArrowLeft } from 'lucide-react';
...
interface ParentRegisterPageProps {
  ...
  onRegister: (parentData: any, childData: any, loginCode: string) => void;
}
...
export function ParentRegisterPage({ onBack, onSwitchToLogin, onRegister }: ParentRegisterPageProps) {
  ...
  const handleRegister = async () => {
    ...
    if (!childAge || parseInt(childAge, 10) < 1 || parseInt(childAge, 10) > 18) {
    ...
    try {
      ...
      onRegister(d.parent, d.child, d.loginCode);
    } catch (e: any) {
      ...
      <div className="m-4 sm:m-5 bg-white/5 rounded-2xl overflow-hidden relative border border-white/10">
        ...
        <div className="px-5 sm:px-7 py-6 sm:py-8 relative z-10">
          ...
          <p className="text-white/80 text-center text-sm sm:text-base mb-5 sm:mb-6">
            Register to create an account for your child
          </p>

          {/* Parent's Full Name */}
          <div className="mb-3 sm:mb-4">
            <label className="block text-white/90 text-xs mb-1.5 px-1">Parent's Full Name</label>
          ...
          {/* Parent's Email */}
          <div className="mb-3 sm:mb-4">
            <label className="block text-white/90 text-xs mb-1.5 px-1">Parent's Email</label>
            <div className="flex items-center bg-white/95 rounded-xl px-4 py-3 gap-3 shadow-sm ring-1 ring-black/5 min-h-[44px]">
          ...
          {/* Confirm Password */}
          <div className="mb-3 sm:mb-4">
            <label className="block text-white/90 text-xs mb-1.5 px-1">Confirm Password</label>
            <div className="flex items-center bg-white/95 rounded-xl px-4 py-3 gap-3 shadow-sm ring-1 ring-black/5 min-h-[44px]">
              <Hash className="w-4 h-4 text-gray-400 flex-shrink-0" strokeWidth={2} />
              <input
                type="password"
          ...
          {/* Child's Name */}
          <div className="mb-3 sm:mb-4">
            <label className="block text-white/90 text-xs mb-1.5 px-1">Child's Name</label>
            <div className="flex items-center bg-white/95 rounded-xl px-4 py-3 gap-3 shadow-sm ring-1 ring-black/5 min-h-[44px]">
              <User className="w-4 h-4 text-gray-400 flex-shrink-0" strokeWidth={2} />
              <input
                type="text"
          ...
          <div className="mb-4 sm:mb-5">
            <label className="block text-white/90 text-xs mb-1.5 px-1">Child's Age</label>
            <div className="flex items-center bg-white/95 rounded-xl px-4 py-3 gap-3 shadow-sm ring-1 ring-black/5 min-h-[44px]">
              <Hash className="w-4 h-4 text-gray-400 flex-shrink-0" strokeWidth={2} />
              <input
                type="text"
