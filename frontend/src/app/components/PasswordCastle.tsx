...
import { SubwayRunnerBackground } from './animations/SubwayRunnerBackground';
import { PasswordLevelRunner } from './animations/PasswordLevelRunner';
import { useRunnerFeedback } from './animations/useRunnerFeedback';
...
export function PasswordCastle({ onClose }: PasswordCastleProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#1e3a5f] via-[#2d5a8a] to-[#1e3a5f] relative overflow-x-hidden">
      ...
      <SubwayRunnerBackground speed={gameState === 'playing' ? 1.35 : 0.75} className="opacity-50" />
...
function Level1({ onComplete, onBack }: { onComplete: () => void; onBack: () => void }) {
  const runner = useRunnerFeedback(25);
  ...
  return (
    <PasswordLevelRunner
      onBack={onBack}
      sceneKey={currentPasswordIndex}
      progress={currentPasswordIndex / passwords.length}
      runnerState={runner.runnerState}
      coinBurstId={runner.coinBurstId}
      scoreGain={runner.scoreGain}
      lane={runner.laneFromIndex(currentPasswordIndex)}
    >
...
function Level2({ onComplete, onBack }: { onComplete: () => void; onBack: () => void }) {
  const runner = useRunnerFeedback(30);
  ...
  return (
    <PasswordLevelRunner
      onBack={onBack}
      sceneKey={currentRound}
      progress={currentRound / rounds.length}
      runnerState={runner.runnerState}
      coinBurstId={runner.coinBurstId}
      scoreGain={runner.scoreGain}
      lane={runner.laneFromIndex(currentRound)}
    >
        <h2 className="text-3xl sm:text-4xl font-bold text-white text-center mb-4">
...
function Level3({ onComplete, onBack }: { onComplete: () => void; onBack: () => void }) {
  const { triggerCorrect, runnerState, coinBurstId, scoreGain, laneFromIndex } = useRunnerFeedback(15);
  ...
  useEffect(() => {
    ...
    if (count > prevMetRef.current) {
      triggerCorrect(10);
    }
    prevMetRef.current = count;
  }, [strength, triggerCorrect]);
  ...
  return (
    <PasswordLevelRunner
      onBack={onBack}
      sceneKey={Object.values(strength).filter(Boolean).length}
      progress={strengthPercentage / 100}
      runnerState={runnerState}
      coinBurstId={coinBurstId}
      scoreGain={scoreGain}
      lane={laneFromIndex(Object.values(strength).filter(Boolean).length)}
    >
        <h2 className="text-3xl sm:text-4xl font-bold text-white text-center mb-4">
...
function Level4({ onComplete, onBack }: { onComplete: () => void; onBack: () => void }) {
  const runner = useRunnerFeedback(35);
  ...
  return (
    <PasswordLevelRunner
      onBack={onBack}
      sceneKey={currentPassword}
      progress={currentPassword / weakPasswords.length}
      runnerState={runner.runnerState}
      coinBurstId={runner.coinBurstId}
      scoreGain={runner.scoreGain}
      lane={runner.laneFromIndex(currentPassword)}
    >
      <AnimatePresence>
        {hackerAttacks > 0 && (
          <motion.div
            ...
            className="absolute top-1/3 -left-4 z-0 pointer-events-none"
          >
        ...
        <h2 className="text-3xl sm:text-4xl font-bold text-white text-center mb-4 relative z-10">
...
function Level5({ onComplete, onBack }: { onComplete: () => void; onBack: () => void }) {
  const runner = useRunnerFeedback(40);
  ...
  return (
    <PasswordLevelRunner
      onBack={onBack}
      sceneKey={currentGate}
      progress={currentGate / gates.length}
      runnerState={runner.runnerState}
      coinBurstId={runner.coinBurstId}
      scoreGain={runner.scoreGain}
      lane={runner.laneFromIndex(currentGate)}
    >
        <h2 className="text-3xl sm:text-4xl font-bold text-white text-center mb-4">
...
function Level6({ onComplete, onBack }: { onComplete: () => void; onBack: () => void }) {
  const { triggerCorrect, triggerWrong, runnerState, coinBurstId, scoreGain, laneFromIndex } = useRunnerFeedback(35);
  ...
  }, [selectedPassword, triggerCorrect, triggerWrong, currentRound, onComplete]);
  ...
  return (
    <PasswordLevelRunner
      ...
      coinBurstId={coinBurstId}
      scoreGain={scoreGain}
      lane={laneFromIndex(currentRound)}
      wide
    >
            ...
            {rounds[currentRound].map((candidate) => {
              ...
              return (
                <motion.button
                  ...
                  disabled={selectedPassword !== null}
                  className={`w-full p-4 rounded-xl text-left transition-all border shadow-md ${
                    isSelected
