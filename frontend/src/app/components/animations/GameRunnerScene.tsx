import { useMemo } from 'react';
import { motion, AnimatePresence } from 'motion/react';
import { SubwayRunnerBackground } from './SubwayRunnerBackground';
import { RunnerCharacter, type RunnerState } from './RunnerCharacter';
import { CoinBurst } from './CoinBurst';
import { ScoreFloater } from './ScoreFloater';
import { slideInUp } from './subwayMotion';

interface GameRunnerSceneProps {
  children: React.ReactNode;
  /** 0–1 question progress within level */
  progress?: number;
  runnerState?: RunnerState;
  lane?: 0 | 1 | 2;
  coinBurstId?: number;
  scoreGain?: number | null;
  sceneKey?: string | number;
  className?: string;
}

export function GameRunnerScene({
  children,
  progress = 0,
  runnerState = 'run',
  lane = 1,
  coinBurstId = 0,
  scoreGain = null,
  sceneKey = 0,
  className = '',
}: GameRunnerSceneProps) {
  const speed = useMemo(() => 1 + progress * 1.2, [progress]);

  return (
    <div className={`relative min-h-[70vh] ${className}`}>
      <SubwayRunnerBackground speed={speed} />
      <RunnerCharacter state={runnerState} lane={lane} />
      <CoinBurst burstId={coinBurstId} />
      <ScoreFloater value={scoreGain} popKey={coinBurstId} />

      <AnimatePresence mode="wait">
        <motion.div key={String(sceneKey)} className="relative z-10" {...slideInUp}>
          {children}
        </motion.div>
      </AnimatePresence>
    </div>
  );
}
